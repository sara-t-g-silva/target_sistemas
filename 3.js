...
function analisarFaturamento(faturamento) {
    ...
    let menorFaturamento = Math.min(...faturamento.map(item => item.valor).filter(valor => valor > 0));
    let maiorFaturamento = Math.max(...faturamento.map(item => item.valor).filter(valor => valor > 0));
    ...
    for (let i = 0; i < faturamento.length; i++) {
        if (faturamento[i].valor > 0) {
            somaFaturamento += faturamento[i].valor;
            diasComFaturamento++;
        }
    }
...
let resultados = analisarFaturamento(faturamentoDiario);
